...
public class DestinationSignalementPublicBD {
    ...
    public long addDestinationSignalementUtilisateurRecu(int idSignalement, int idUtilisateur) {
        ...
        return db.insert(TABLE_NAME_DESTINATION_SIGNALEMENT_UTILISATEUR_RECU, null, values);
    }

    public long addDestinationSignalementUtilisateurAEnvoyer(int idSignalement, int idUtilisateur) {
        // Ajout d'un enregistrement dans la table
        ...
        // insert() retourne l'id du nouvel enregistrement inséré, ou -1 en cas d'erreur
        return db.insert(TABLE_NAME_DESTINATION_SIGNALEMENT_UTILISATEUR_A_ENVOYER, null, values);
    }

    public int deleteDestinationSignalementUtilisateurRecu(int idSignalement) {
        ...
        return db.delete(TABLE_NAME_DESTINATION_SIGNALEMENT_UTILISATEUR_RECU, where, whereArgs);
    }

    public int deleteDestinationSignalementUtilisateurAEnvoyer(int idSignalement) {
        // suppression d'un enregistrement
        // valeur de retour : (int) nombre de lignes affectées par la clause WHERE, 0 sinon
    ...
    public ArrayList<Signalement> getSignalementAEnvoyer(int idUtilisateur) {
        // sélection de tous les enregistrements de la table


        Cursor c = db.rawQuery("SELECT * FROM "+TABLE_NAME_DESTINATION_SIGNALEMENT_UTILISATEUR_A_ENVOYER+", "+SignalementBD.TABLE_NAME_SIGNALEMENT_A_ENVOYER+", "+ArretBD.TABLE_NAME+", "+UtilisateurBD.TABLE_NAME+", "+TypeSignalementBD.TABLE_NAME+" WHERE "+ID_UTILISATEUR_DESTINATION+"="+idUtilisateur+" AND "+ID_SIGNALEMENT_UTILISATEUR+"="+SignalementBD.ID_SIGNALEMENT+" AND "+SignalementBD.ARRET_SIGNALEMENT+"="+ArretBD.ID_ARRET+" AND "+SignalementBD.EMETTEUR_SIGNALEMENT+"="+UtilisateurBD.ID_UTILISATEUR+" AND "+SignalementBD.TYPE_SIGNALEMENT+"="+TypeSignalementBD.ID_TYPE_SIGNALEMENT, null);
        ...
        if (c.moveToFirst()) {
            while (c.isAfterLast() == false) {
                ...
                Signalement s = new SignalementPublic(0,"","",null,null,null,null,null);
    ...
    public ArrayList<Utilisateur> getUtilisateurSignalementRecu(int idSignalement) {
        // sélection de tous les enregistrements de la table
        Cursor c = db.rawQuery("SELECT * FROM "+TABLE_NAME_DESTINATION_SIGNALEMENT_UTILISATEUR_RECU+", "+UtilisateurBD.TABLE_NAME+" WHERE "+ID_SIGNALEMENT_UTILISATEUR+"="+idSignalement+" AND "+ID_UTILISATEUR_DESTINATION+"="+UtilisateurBD.ID_UTILISATEUR, null);
    ...
    public ArrayList<Utilisateur> getUtilisateurSignalementAEnvoyer(int idSignalement) {
        // sélection de tous les enregistrements de la table

        Cursor c = db.rawQuery("SELECT * FROM "+TABLE_NAME_DESTINATION_SIGNALEMENT_UTILISATEUR_A_ENVOYER+", "+UtilisateurBD.TABLE_NAME+" WHERE "+ID_SIGNALEMENT_UTILISATEUR+"="+idSignalement+" AND "+ID_UTILISATEUR_DESTINATION+"="+UtilisateurBD.ID_UTILISATEUR, null);
